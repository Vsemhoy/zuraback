...
class TaskConversationController extends Controller
{
    ...
    public function storeComment(StoreTaskCommentRequest $request, Scope $scope, Task $task): CommentResource
    {
        ...
        $comment = $task->comments()->create([...$data, 'scope_id' => $scope->id, 'created_by' => $request->user()->id]);
        ActivityLog::query()->create([
            'scope_id' => $scope->id, 'actor_id' => $request->user()->id, 'subject_type' => 'task', 'subject_id' => $task->id,
            'action' => 'task.comment_created', 'after' => ['comment_id' => $comment->id], 'context' => ['comment_id' => $comment->id],
